...
angular.module('itapapersApp')
  .controller('AuthorCtrl', ['$scope', '$stateParams', '$timeout', 'store', 'server', 'hudson', 'debug', 'documentTypes', 'utils', 'csv', 'colours', function ($scope, $stateParams, $timeout, store, server, hudson, debug, documentTypes, utils, csv, colours) {
...
//          var hIndex = citationCount["h-index"] ? citationCount["h-index"][0] : 0;
    ...
    $scope.addCitations = function() {
      var citations = prompt("Please enter the citation count, h-index and URL separated with spaces", "123 13 www.url.com");
      if (citations !== null) {
        ...
        var ce;
        var split = citations.split(" ");
        var date = new Date();
        var formattedDate = date.getDate() + "/" + (date.getMonth() + 1) + "/" + date.getFullYear();

        if (split.length !== 3) {
          // generate blank citation
          ce = "the person citation count 'citation:" + $stateParams.authorId + "-01'" +
            "\n  has '" + formattedDate + "' as date checked.";

          ce += "\n\nthe published person '" + $stateParams.authorId + "'" +
            "\n  has the person citation count 'citation:" + $stateParams.authorId + "-01' as citation count.";
        } else {
          // generate CE
          var citationCount = split[0];
          var hIndex = split[1];
          var scholarLink = split[2];

          ce = "the person citation count 'citation:" + $stateParams.authorId + "-01'" +
            "\n  has '" + scholarLink + "' as url and" +
            "\n  has '" + formattedDate + "' as date checked and" +
            "\n  has '" + citationCount + "' as citation count and" +
            "\n  has '" + hIndex + "' as h-index.";
